...
Note that you can get a markdown/html version by adding the command `kableone`.
...
However, to get a markdown (styled) output, you need to run your script in a Quarto document,
not in an R script.
...
Check-out the [TableOne manual](https://cran.r-project.org/web/packages/tableone/vignettes/introduction.html) for a more detailed introduction.


How to export to MS Office? Well, one simple approach is obviously to copy-paste (the styled version). 
...
describe_distribution(penguins) |>  # describe all distributions in the data set
  format_table(digits = 2)  # show 2 decimals only
```
