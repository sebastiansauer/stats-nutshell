...
### The linear model in all its glory
...
### A ladder of common regression formula variants
...
- `_c`: a *c*entered variable


0. `y ~ 1` - the simple grand mean value
1. `y ~ b` - difference between two mean values
2. `y ~ g` - differences between multiple mean values
3. `y ~ x_c` - y as a linear function of x
4.  `y ~ x1_c + x2_c` - y as a linear function of both x1 and x2
5. `y ~ x_c + g + x_c:g`  - the interaction (moderator) model
6. `log(y) ~ x1 + x2` - multiplicative association of independent variables
7. `log(p/(1-p)) ~ x` - logistic regression





## A simple model: one metric predictor
...
See [Datenwerk](https://sebastiansauer.github.io/Datenwerk/#category=regression) with tag "#regression".
...
However, unfortunately, the plot of the model *needs* a nominal variable if we are to compare groups. 
In our case, `am` is considered a numeric variables, since it consists of numbers only. 
The plot does not work as expected, malheureusement:
...
We need to transform `am` to a *factor* variable. 
That's something like a string (text) variable. 
If we hand over a `factor()` to the plotting function, 
...
If we were not to specify the values of `am` which we would like to get predictions for, 
the default of the function would select 10 values, 
spread across the range of `am`. For numeric variables, this is usually fine. 
However, for nominal variables - and `am` is in fact a nominally scaled variable - 
we insist that we want predictions 
for the levels of the variable only, that is for `0` and `1`.
...
### Quadrat models are also linear
...
Note `x^2` in a regression formula is *not* understood as arithmetic squaring.
Hence, we need the `I()`.
...
Again,
here's the scatter plot (for `mpg` as a function of `disp`):
